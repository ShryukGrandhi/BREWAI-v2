...
class BrowserUseClient:
    ...
    def __init__(self, api_key: str, gemini_api_key: str):
        ...
        try:
            ...
            # Add provider attribute for BrowserUse compatibility
            if not hasattr(base_llm, 'provider'):
                base_llm.provider = "google"
...
def get_browseruse_client() -> BrowserUseClient:
    ...
    # If browser-use is not available, use mock implementation
    if not HAS_BROWSER_USE:
        from services.browseruse_client_mock import get_browseruse_client as get_mock_client
